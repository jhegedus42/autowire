...
object Macros {
  ...
  : c.Expr[Future[Result]] = {
    ...
    val res = for {
      q"${Pkg(_)}.`package`.$callableName[$t]($contents)" <- Win(c.prefix.tree,
                                                                 "You can only use .call() on the Proxy returned by autowire.Client.apply, not " + c
                                                                                                                                                   .prefix
                                                                                                                                                   .tree
                                                                )
          ...
          path match {
            case Nil         =>
              lastMem match {
                ...
                case Some(mem)                 => c.abort(c.enclosingPosition,
  ...
  : c.Expr[Router[PickleType]] = {
    ...
    val routes = help.getAllRoutesForClass(pt, target, topClass, topClass.typeSymbol.fullName.toString.split('.').toSeq,
                                           Nil).toList

    val res = q"{case ..$routes}: autowire.Core.Router[$pt]"
    println("RES", res)
    val e: c.Expr[Router[PickleType]] = c.Expr(res)
    e
  }
  ...
  case
  class Luz[T](s: String) extends Check[T] {
  ...
  case
  class Win[T](t: T, s: String) extends Check[T] {
  ...
  class MacroHelp[C <: Context](val c: C) {
    ...
    def getValsOrMeths(curCls: Type): Iterable[Either[(c.Symbol, MethodSymbol), (c.Symbol, MethodSymbol)]] = {
      ...
      val membersOfBaseAndParents: Iterable[Symbol] = curCls.declarations ++ curCls.baseClasses
                                                                             .map(_.asClass.toType.declarations).flatten
    ...
    def getAllRoutesForClass(
      ...
      getValsOrMeths(curCls).flatMap {
                                       case Left((m, t))  => Nil
                                         //Vals / Vars
                                         getAllRoutesForClass(pt, target, m.typeSignature, prefixPath,
                                                              memPath :+ m.name.toString)
                                       case Right((m, t)) =>
                                         //Methods
                                         Seq(extractMethod(pt, t, prefixPath, memPath :+ m.name.toString, target,
                                                           curCls))
